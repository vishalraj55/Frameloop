    ...
    <div className="max-w-117.5 mx-auto bg-black text-white min-h-screen">
      ...
      <div className="sticky top-0 z-10 bg-black/90 backdrop-blur-md border-b border-[#1f1f1f]">
        <div className="flex items-center gap-4 px-4 pt-4 pb-3">
          ...
          <span className="text-[15px] font-semibold tracking-tight">{username}</span>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center mt-20 gap-2">
          <p className="text-[14px] font-semibold text-white">No followers yet</p>
          <p className="text-[13px] text-[#555]">When someone follows, they&apos;ll appear here</p>
        </div>
      ) : (
        <ul className="divide-y divide-[#1a1a1a]">
          {filtered.map((user) => (
            <li key={user.id} className="flex items-center gap-3 px-4 py-3 hover:bg-white/2 transition-colors">
              <button
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  <p className="text-[13px] font-semibold leading-tight">{user.username}</p>
                  {user.bio && (
                    <p className="text-[12px] text-[#666] truncate mt-0.5 leading-tight">{user.bio}</p>
                  )}
                </button>
              </div>
